...
On Render, set `FRONTEND_URL` to your frontend URL(s), comma-separated (e.g. `https://tracker.mulkinternational.co,https://po-tracker-eight.vercel.app,https://cynergy.mulkinternational.co`) for CORS. The API also allows `*.mulkinternational.co` and `*.vercel.app` origins.
